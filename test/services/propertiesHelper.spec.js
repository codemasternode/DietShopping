...
describe("test function checkIsObjectHasRequiredProperties", () => {
    it("should throw error when doesn't pass required parameters", (done) => {
        expect(() => checkIsObjectHasRequiredProperties()).to.throw()
        done()
    })
    it("should throw error when arrayOfRequiredProperties is not an array", (done) => {
        expect(() => checkIsObjectHasRequiredProperties({ a: "a" })).to.throw("arrayOfRequiredProperties is an array")
        done()
    })
    it("should throw error when objectToCheck is not a object", (done) => {
        expect(() => checkIsObjectHasRequiredProperties(["a"])).to.throw("objectToCheck is a object")
        done()
    })
    it("should throw error when arrayOfRequiredProperties is not array with strings only", (done) => {
